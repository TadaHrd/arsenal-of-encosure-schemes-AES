...
let double_anyway_sep_check_regex = /^[^aesinywAESINYW*\\ ]+$/;
let double_anyway_sep_regex = /[^aesinywAESINYW*\\ ]+/;
...
function bytesToWords(byteArray) {
    ...
    for (let i = 0; i < byteArray.length; i += 2) {
        ...
        else wordArray.push(byteArray[i]);
    }
    return wordArray;
}

function escaped_double_anyway_encode(input, sep) {
...
function double_anyway_encode(input, sep, escape = false) {
    ...
    for (let val of words) {
        ...
        if (escape) line = fix1esc + fix1;
        else line = fix1;

        line += chr(ord("A") + 32 * (val > 256));
        line += chr(ord("E") + 32 * ((body >> 0) & 1));
        line += chr(ord("S") + 32 * ((body >> 1) & 1));

        if (escape) line += fix1 + fix1esc + " " + fix2esc + fix2;
        else line += fix1 + " " + fix2;

        line += chr(ord("I") + 32 * ((body >> 2) & 1));
        line += chr(ord("S") + 32 * ((body >> 3) & 1));

        if (escape) line += fix2 + fix2esc + " " + fix3esc + fix3;
        else line += fix2 + " " + fix3;

        line += chr(ord("A") + 32 * ((body >> 4) & 1));
        line += chr(ord("N") + 32 * ((body >> 5) & 1));
        line += chr(ord("Y") + 32 * ((body >> 6) & 1));
        line += chr(ord("W") + 32 * ((body >> 7) & 1));
        line += chr(ord("A") + 32 * ((body >> 8) & 1));
        line += chr(ord("Y") + 32 * ((body >> 9) & 1));

        if (escape) line = line + fix3 + fix3esc;
        else line = line + fix3;
...
function escaped_double_anyway_decode(input, sep) {
    return double_anyway_decode(input, sep);
}

function double_anyway_decode(text, return_string) {
    let ret = [];

    for (let line of text.trim().split(double_anyway_sep_regex)) {
        line = line.replace(/\\\*/g, "");
        ...
        let num = 0;
        for (let i = 0; i < 16; i++) num += Num(bits[i]) << i;

        if (dual_byte) ret.push(num >> 8);
        ret.push(num & 0b1111_1111);
    }

    if (return_string) return textDecoder.decode(new Uint8Array(ret));

    return ret;
}

if (window.encosureSchemes instanceof Set) window.encosureSchemes.add("AdES");
